...
The extractor reads the rendered archive tables, follows only HTTPS links on the DoE object-storage
host, validates PDF/DOCX signatures, and records SHA-256 hashes. Routine updates reuse source-level
rows already present in the processed tables, download only new or replaced reports, and delete report
files after successful extraction. Raw documents can be downloaded again from their preserved source
URLs and verified against the recorded hashes. Older monthly pages supply the year in the page title
while rows contain only month names; `source_label` states when the year was inherited from the official
year page.
...
## Population, HDI, and tree-cover context

All files in `data/context/` describe Bangladesh nationally, not Dhaka city. Their workbook sheets preserve this scope explicitly.
...
- Tree-cover loss: Global Forest Watch (2025), distributed by Our World in Data as `tree-cover-loss.csv`. <https://ourworldindata.org/grapher/tree-cover-loss>
- Original Global Forest Watch/WRI driver dataset. <https://datasets.wri.org/datasets/dominant-drivers-of-tree-cover-loss-at-1km>
...
The tree-cover CSV was retrieved on 6 August 2026 from the published Our World in Data download and verified with SHA-256 `30944f32b9b9829bcc7d666b46f2db6b141c743b0742e447d514ef39f3f13440`. It covers 2001–2024. The metric is stand-replacement tree-cover disturbance in 30 m pixels from all causes, not necessarily permanent deforestation. It is retained as national descriptive context and is not correlated with monthly Dhaka pollution.
