...
FILE1 = BASE_DIR / "data" / "raw" / "dhaka_aqi_monthly_enriched_v2_2017_2025_filled.xlsx"
FILE2 = BASE_DIR / "data" / "raw" / "dhaka_observed_air_quality_dataset.xlsx"
...
def export_outputs(
    ...
    """Write all output artefacts into output_dir and data/processed/."""
    output_dir.mkdir(parents=True, exist_ok=True)
    DATA_PROCESSED_DIR.mkdir(parents=True, exist_ok=True)

    # -- final dataset CSV & Excel → data/processed/ -----------------------
    final.to_csv(DATA_PROCESSED_DIR / "final_dhaka_aqi_dataset.csv", index=False)
    final.to_excel(DATA_PROCESSED_DIR / "final_dhaka_aqi_dataset.xlsx", index=False, engine="openpyxl")
    ...
    # -- correlation matrix -------------------------------------------------
    _write_correlation_matrix(final, output_dir)

    print(f"\n✅ All outputs written to {output_dir}/ and {DATA_PROCESSED_DIR}/")
...
def _write_correlation_matrix(df: pd.DataFrame, out: Path) -> None:
    """Compute and export a correlation matrix of key numeric variables."""
    numeric_cols = [
        "aqi_mean", "aqi_median", "aqi_min", "aqi_max",
        "pm25_mean", "pm25_median", "pm25_min", "pm25_max",
        "hourly_observations", "coverage_pct",
        "population_total", "urban_population", "rural_population",
        "urban_share_pct", "rural_share_pct",
        "hdi", "poverty_rate_pct",
    ]
...
def generate_figures(df: pd.DataFrame) -> None:
    """Create 7 publication-ready figures in outputs/figures/."""
    import matplotlib
    matplotlib.use("Agg")
    ...
    # 1. Monthly AQI trend 2017-2025
    fig, ax = plt.subplots(figsize=(12, 6))
    ax.plot(df["month_start"], df["aqi_mean"], color="#e74c3c", linewidth=1.5)
    ax.set_title("Monthly Mean AQI – Dhaka (2017–2025)", fontsize=14, fontweight="bold")
    ax.set_xlabel("Date")
    ax.set_ylabel("AQI (Mean)")
    fig.tight_layout()
    fig.savefig(fig_dir / "monthly_aqi_trend.png", dpi=300)
    plt.close(fig)

    # 2. Monthly PM2.5 trend 2017-2025
    fig, ax = plt.subplots(figsize=(12, 6))
    ax.plot(df["month_start"], df["pm25_mean"], color="#2980b9", linewidth=1.5)
    ax.set_title("Monthly Mean PM2.5 – Dhaka (2017–2025)", fontsize=14, fontweight="bold")
    ax.set_xlabel("Date")
    ax.set_ylabel("PM2.5 (µg/m³)")
    fig.tight_layout()
    fig.savefig(fig_dir / "monthly_pm25_trend.png", dpi=300)
    plt.close(fig)

    # 3. AQI by month boxplot (seasonality)
    fig, ax = plt.subplots(figsize=(14, 6))
    sns.boxplot(
        data=df, x="month_name", y="aqi_mean", hue="month_name",
        order=month_order, palette="Reds", legend=False, ax=ax,
    )
    ax.set_title("AQI Seasonality by Month – Dhaka (2017–2025)", fontsize=14, fontweight="bold")
    ax.set_xlabel("Month")
    ax.set_ylabel("AQI (Mean)")
    ax.tick_params(axis="x", rotation=45)
    fig.tight_layout()
    fig.savefig(fig_dir / "aqi_seasonality.png", dpi=300)
    plt.close(fig)

    # 4. PM2.5 by month boxplot (seasonality)
    fig, ax = plt.subplots(figsize=(14, 6))
    sns.boxplot(
        data=df, x="month_name", y="pm25_mean", hue="month_name",
        order=month_order, palette="Blues", legend=False, ax=ax,
    )
    ax.set_title("PM2.5 Seasonality by Month – Dhaka (2017–2025)", fontsize=14, fontweight="bold")
    ax.set_xlabel("Month")
    ax.set_ylabel("PM2.5 (µg/m³)")
    ax.tick_params(axis="x", rotation=45)
    fig.tight_layout()
    fig.savefig(fig_dir / "pm25_seasonality.png", dpi=300)
    plt.close(fig)

    # 5. Year-by-year AQI boxplots
    fig, ax = plt.subplots(figsize=(14, 6))
    sns.boxplot(
        data=df, x="year", y="aqi_mean", hue="year",
        palette="RdYlGn_r", legend=False, ax=ax,
    )
    ax.set_title("Year-by-Year AQI Distribution – Dhaka (2017–2025)", fontsize=14, fontweight="bold")
    ax.set_xlabel("Year")
    ax.set_ylabel("AQI (Mean)")
    fig.tight_layout()
    fig.savefig(fig_dir / "aqi_yearly_boxplot.png", dpi=300)
    plt.close(fig)

    # 6. Year-by-year PM2.5 boxplots
    fig, ax = plt.subplots(figsize=(14, 6))
    sns.boxplot(
        data=df, x="year", y="pm25_mean", hue="year",
        palette="YlOrBr", legend=False, ax=ax,
    )
    ax.set_title("Year-by-Year PM2.5 Distribution – Dhaka (2017–2025)", fontsize=14, fontweight="bold")
    ax.set_xlabel("Year")
    ax.set_ylabel("PM2.5 (µg/m³)")
    fig.tight_layout()
    fig.savefig(fig_dir / "pm25_yearly_boxplot.png", dpi=300)
    plt.close(fig)

    # 7. Correlation heatmap
    corr_cols = [
        "aqi_mean", "pm25_mean", "hourly_observations", "coverage_pct",
        "population_total", "urban_share_pct", "hdi", "poverty_rate_pct",
    ]
    for col in ("norm_wind", "norm_rain"):
        if col in df.columns:
